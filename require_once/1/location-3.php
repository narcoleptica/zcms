...
class Z_LOCATION
{
	...
	{
		return (string)(preg_match("'" . self::$toPATTERN[self::PRE] . "'x", (string)$new) ? $new : $old);
	}
	...
	final static private function toHOST(&$old, $new = null, $flag = 0)
	{
		...
		{
			$host = ($f === 0) ? array() : $old;
			$PATTERN = "'" . self::$toPATTERN[self::HOST] . "'x";
			foreach($new as $d)
			{
				$d = (string)$d;
	...
	final static private function toPATH(&$old, $new = null, $flag = 0)
	{
		...
		{
			$path = ($f === 0) ? array() : $old;
			$PATTERN = "'" . self::$toPATTERN[self::PATH] . "'x";
			foreach($new as $d)
			{
				$d = (string)$d;
				if (!(strlen($d) > 0)) continue;
		...
		{
			$path = (string)$new;
			$path = (string)preg_replace("'" . '^ [\/]? (.*?) [\/]? (?: [\?] .* )? $' . "'x", '\\1', (string)$new);
			$path = split('\\/', $path);
	...
	final static private function toDATA(&$old, $new = null, $flag = 0) // flag > 0 - новов значене актуальнее
	{
		...
		{
			$data = ($f === 0) ? array() : $old;
			$PATTERN = "'" . self::$toPATTERN[self::DATA] . "'x";
	...
	{
		return (string)(preg_match("'" . self::$toPATTERN[self::POST] . "'x", (string)$new) ? $new : $old);
	}
	...
	final static private function initingDefault()
	{
		...
		try
		{
			...
			if (array_key_exists($s5, $S)) $D[self::ROOT] = preg_replace("'" . ' [\/]+ $' . "'x", '', (string)$S[$s5]);
		}
		catch (Exception $E) { }
		//
		try
	...
	final public function initing($new = null, $flag = null)
	{
		if (is_array($new))
		{
			...
			$flag = (string)$flag;
			$fH = preg_match("'H [\-\+]'ix", $flag) ? preg_replace("'" . '^ .*? H (\-|\+) .*$' . "'ix", '${1}1', $flag) : 0;
			$fP = preg_match("'P [\-\+]'ix", $flag) ? preg_replace("'" . '^ .*? P (\-|\+) .*$' . "'ix", '${1}1', $flag) : 0;
			$fD = preg_match("'D [\-\+]'ix", $flag) ? preg_replace("'" . '^ .*? D (\-|\+) .*$' . "'ix", '${1}1', $flag) : 0;
			// pre
	...
	final static private function dataByKeysToId(&$data, $keys = null, $default = null, $value = null)
	{
		try
		{
			$return = self::__data($data, $keys);
			if (!preg_match("'" . '^ [1-9] [\d]{0,19} $' . "'x", $return)) throw new Exception('wrong ID format');
	...
	final static private function dataByKeysToNumber(&$data, $keys = null, $default = null, $value = null)
	{
		try
		{
			...
			if (!preg_match("'" . '^ [\-\+]? [1-9] [\d]*$' . "'x", $return)) throw new Exception('wrong NUMBER format');
			if ($value !== null) return ($return == $value) ? true : false;
			return $return;
		}
		catch (Exception $E) { return $default; }
