...
class Z_LOCATION
{
	...
	final static private function array_reorder($array) { static $F = null; if ($F === null) $F = create_function('$value, $key, $data', '$data[0][$data[1]] = $value; $data[1]++;'); $A = array(); $i = 0; array_walk($array, $F, array(0 => &$A, 1 => &$i)); return $A; }

	final public function unsetHOST($index = null) { $C = &$this->constantData[self::HOST]; if(array_key_exists($index, $C)) { unset($C[$index]); $C = self::array_reorder($C); return true; } return false; }

	final public function unsetPATH($index = null) { $C = &$this->constantData[self::PATH]; if(array_key_exists($index, $C)) { unset($C[$index]); $C = self::array_reorder($C); return true; } return false; }

	final public function unsetDATA($index = null) { $C = &$this->constantData[self::DATA]; if(array_key_exists($index, $C)) { unset($C[$index]); return true; } return false; }
	...
	final public function uh($index = null) { return $this->unsetHOST($index); }
	...
	final public function ur($index = null) { return $this->unsetPATH($index); }
	...
	final public function ug($index = null) { return $this->unsetDATA($index); }
