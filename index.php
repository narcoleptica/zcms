...
function templateHtml($file) { return new XTemplate($file); }
function pagesInit($data = null)
{
	$data = is_array($data) ? $data : array();
	$item_count = isset($data['item_count']) ? max(0, (int)$data['item_count']) : 0;
	$item_at_page = isset($data['item_at_page']) ? max(1, (int)$data['item_at_page']) : 10;
	$page = isset($data['page']) ? max(0, (int)$data['page']) : 0;
	$page_count = (int)ceil($item_count / $item_at_page);
	$page_range = 1;
	$item_first = $page * $item_at_page;
	return array(
			'item_at_page' => $item_at_page,
...
function pagesHtml($module, $data = array(), $file = 'resource/index.htm')
{
	$data = is_array($data) ? $data : array();
	$item_count = isset($data['item_count']) ? max(0, (int)$data['item_count']) : 0;
	$item_at_page = isset($data['item_at_page']) ? max(1, (int)$data['item_at_page']) : 10;
	$page = isset($data['page']) ? max(0, (int)$data['page']) : 0;
	$page_count = isset($data['page_count']) ? max(0, (int)$data['page_count']) : (int)ceil($item_count / $item_at_page);
	$page_range = isset($data['page_range']) ? max(0, (int)$data['page_range']) : 1;
	$item_first = isset($data['item_first']) ? max(0, (int)$data['item_first']) : (int)($page * $item_at_page);
	// no pages needed
	if (!($page_count > 1)) return false;
	// pages && goto: init
	$T = new XTemplate($file);
	$pages = array(0);
	if (($page - $page_range) > 1) $pages[] = null; // left "..."
	for ($i = max($page - $page_range, 1), $I = min($page + $page_range, $page_count - 2); $i <= $I; $i++) { $pages[] = $i; }
	if (($page + $page_range) < $page_count - 2) $pages[] = null; // right "..."
	$pages[] = $page_count - 1;
	// pages && goto: parse
	$T->assign('module', $module);
	foreach ($pages as $p)
	{
		if ($p === null) $T->insert_loop('pages.page.delimiter', array());
		elseif ($p === $page)
		{
			$T->insert_loop('pages.page.current', array('title'=>$p + 1));
			$T->insert_loop('pages.goto', array('value'=>$p + 1));
		}
		else $T->insert_loop('pages.page.link', array('value'=>$p + 1, 'title'=>$p + 1));
		$T->insert_loop('pages.page', array());
	}
	$T->parse('pages');
	return $T->text('pages');
}
...
//
session_start();
$MY = isset($_SESSION['MY']) && is_array($_SESSION['MY']) ? $_SESSION['MY'] : array('page'=>0);
try
{
	$module = $L->pS(array('ajax'), false, -1);
	if ($module !== false)
	{
		header('Content-Type: text/plain; charset=windows-1251');
		if ($module === 'tracking') $module = 'modules/tracking/tracking.php';
		else $module = 'modules/tracking/tracking.php';
		require_once($module);
		throw new Exception('unknown exception');
	}
	header('Content-Type: text/html; charset=windows-1251');
	$T = templateHtml('resource/index.htm');
	$DATA = array(
			'description' => '',
			'keywords' => '',
			'title' => '',
			'content' => '',
			'module' => isset($MY['module']) && is_string($MY['module']) ? $MY['module'] : 'tracking'
		);
	$T->assign('DATA', $DATA);
	$T->parse('index');
	print $T->text('index');
